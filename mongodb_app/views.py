...
def sendback(request):
    ...
    enginesize = request.POST['enginesize']
    horsepower = request.POST['horsepower']
    wheelbase = request.POST['wheelbase']
    wid = request.POST['width']
    le = request.POST['length']
    curbweight = request.POST['curbweight']
    fuelcapacity = request.POST['fuelcapacity']
    fuelefficiency = request.POST['fuelefficiency']
    ...
    data = pd.read_csv('mongodb_app/static/csvfile/Car_sales.csv')
    data.head()
    ...
    li.append(year)
    li.append(price)
    li.append(enginesize)
    li.append(horsepower)
    li.append(wheelbase)
    li.append(wid)
    li.append(le)
    li.append(curbweight)
    li.append(fuelcapacity)
    li.append(fuelefficiency)
    li.append(power)
    for i in li:
        i=float(i)
    xi=[] #to make list 2-d 
